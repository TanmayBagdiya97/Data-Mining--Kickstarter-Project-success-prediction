...
train_x['Quarter'] <- ceiling(as.numeric(train_x$launch_month) / 3)
...
train <- train_x %>%
  ...
  mutate(count=n())%>%
  ungroup()%>%
  mutate(category_name=if_else(count<1000,'Other',category_name))%>%
  mutate(location_slug =if_else(str_sub(location_slug, start= -2)=="or",str_sub(location_slug, start= -2),"US"))%>%
  ...
  mutate(isbwImg1=ifelse (is.na(isbwImg1),2,isbwImg1),
         ...
         accent_color=ifelse (is.na(accent_color),'NULL',accent_color),
  ...
  mutate (
    grade_level = ifelse(is.na(grade_level), mean(grade_level, na.rm=TRUE), grade_level)) %>%
  group_by(grade_level)%>%
  mutate(
    grade_level = ntile(grade_level, 4),
...
# vocab_small = prune_vocabulary(vocab, vocab_term_max = 10)
# # 
...
train_success <- train %>%
  select(-c(launch_month,launched_at,deadline,accent_color,color_foreground,color_background,id,big_hit,creator_id, backers_count,creator_name,captions,name,tag_names,blurb,reward_descriptions,count,reward_amounts,afinn_pos,afinn_neg))
